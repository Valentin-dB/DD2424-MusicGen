...
def label_sequences_transformer(x,sequence_length,char_to_ind=None,n_batch=1):
    n = x.shape[0]
    n_samples = (n-1)//sequence_length
    n_samples = (n_samples//n_batch)*n_batch
    training_set = np.reshape(x[:n_samples*sequence_length],(n_samples,sequence_length))
    if char_to_ind is not None :
        x = one_hot_encode(char_to_ind,x)
...
def prep_data_transformer(seq_length,ClassicSongs,n_Channels,n_Notes,channel_to_ind,note_to_ind,val_split=0.1,test_split=0.1):
    if type(ClassicSongs) == list : ClassicSongs = files_to_songs(ClassicSongs)
    ...
    current_index = 0
    for song in ClassicSongs.values():
        song_x_channels, song_y_channels = label_sequences_transformer(song[:, 0],seq_length, channel_to_ind)
        song_x_notes, song_y_notes = label_sequences_transformer(song[:, 1], seq_length, note_to_ind)
        song_x_velocities, song_y_velocities = label_sequences_transformer(song[:, 2], seq_length)
